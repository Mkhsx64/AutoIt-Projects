...
Local $aAccelKeys[23][23] = [["{TAB}", $eTab], ["^s", $fSave], ["^o", $fOpen], ["^a", $eSA], ["^f", $eFind], ["^h", $eReplace], ["^p", $fPrint], ["^n", $fNew], ["^w", $eWC], ["^l", $eLC], ["^+u", $eSU], ["^+l", $eSL], ["^+s", $fSaveAs], ["^r", $eRedo], ["{F5}", $eTD], ["{F2}", $hVHelp], ["^+a", $mCombo[1]], ["^+h", $mCombo[2]], ["^+q", $mCombo[0]], ["{F7}", $forComp], ["{F3}", $vTxt_Spch], ["{F4}", $vSE], ["{F1}", $au3help]]
...
$au3tool_Pos = WinGetPos("[ACTIVE]")
ToolTip("Au3 Syntax On", $au3tool_Pos[0], $au3tool_Pos[1])
Sleep(1000)
ToolTip("")
Else
...
$au3Count = 0
$au3tool_Pos = WinGetPos("[ACTIVE]")
ToolTip("Au3 Syntax Off", $au3tool_Pos[0], $au3tool_Pos[1])
Sleep(1000)
ToolTip("")
EndIf
Case $forComp
...
$au3help = GUICtrlCreateMenuItem("AutoIt Help" & @TAB & "F1", $HelpM, 0)
$hVHelp = GUICtrlCreateMenuItem("View Help" & @TAB & "F2", $HelpM, 1)
GUICtrlCreateMenuItem("", $HelpM, 2)
$hAA = GUICtrlCreateMenuItem("About AuPad", $HelpM, 3)
$hVH = GUICtrlCreateMenuItem("Version History", $HelpM, 4)
setNew()
GUISetState(@SW_SHOW)
EndFunc
Func STGUI()
_GUICtrlRichEdit_Destroy($pEditWindow)
$pEditWindow = _GUICtrlRichEdit_Create($pWnd, "", 0, 0, 600, 460, BitOR($ES_MULTILINE, $WS_VSCROLL, $ES_AUTOVSCROLL))
GUICtrlSetState($vStatus, $GUI_CHECKED)
$Status_Label = GUICtrlCreateLabel("Ln: " & "   |  Col: ", 480, 460, 1000)
AdlibRegister("chkLineCol", 1000)
GUISetState()
EndFunc
Func chkLineCol()
Local $lineCount, $CharPos = _GUICtrlRichEdit_GetSel($pEditWindow), $ColCount = _GUICtrlRichEdit_GetLineLength($pEditWindow, _GUICtrlRichEdit_GetLineNumberFromCharPos($pEditWindow, $CharPos[0]))
$lineCount = _GUICtrlRichEdit_GetLineNumberFromCharPos($pEditWindow, $CharPos[0])
GUICtrlSetData($Status_Label, "Ln:" & $lineCount & "   |  Col:" & $ColCount)
EndFunc
Func NSTGUI()
_GUICtrlRichEdit_Destroy($pEditWindow)
$pEditWindow = _GUICtrlRichEdit_Create($pWnd, "", 0, 0, 600, 480, BitOR($ES_MULTILINE, $WS_VSCROLL, $ES_AUTOVSCROLL))
GUICtrlSetState($vStatus, $GUI_UNCHECKED)
AdlibUnRegister("chkLineCol")
GUICtrlDelete($Status_Label)
EndFunc
Func cGUI()
...
If $searchText <> "" Then
$seInput = GUICtrlCreateInput($searchText, 8, 55)
Else
...
GUICtrlSetState($seSubmit, $GUI_DEFBUTTON)
GUISetState()
EndFunc
...
_GUICtrlRichEdit_SetSel($pEditWindow, $gSel[0], $gSel[1], False)
EndFunc
Func setNew()
