...
Local $ButtonFTP1, $Update = 0, $ButtonFTP2, $sUser, $ServerVersion = "1.20"
Local $hChild_2
	...
	Switch $msg[1]
		Case $GUIhandle
			Switch $msg[0]
				...
				Case $ButtonFTP2
					$Update = 0
					MsgBox(0, "FTP", "You've turned off auto updates.", 5)
			EndSwitch
		Case $hChild_1
			Switch $msg[0]
				Case $GUI_EVENT_CLOSE
...
Func ParseLineData($LineReceived)
	...
	Local $Version, $UpdateClient
	;ConsoleWrite($LineReceived & @CRLF)
	;MsgBox(0, "", $LineReceived & @CRLF)
	$Version = IniRead("UpdateConfig.ini", "1", "Version", "")
	...
	;This is where we check to see if autoUpdate is on; Check to see if the version is in the line; and check the version number.
	If StringInStr($Data, "Version:") <> 0 And StringInStr($Data, $Version) = 0 And $Update = 1 Then
		;MsgBox(0, "", "data: " & $Data & $Version, 1)
		ConsoleWrite(@CRLF & $Data)
		$UpdateClient = TCPSend($sUser, "UPDATE")
		If @error Then
			MsgBox(0, "", "No Success")
		Else
			MsgBox(0, "", "Success", 1)
...
Func GUI()
	...
	$ButtonFTP1 = GUICtrlCreateButton("Update On", 120, 486) ;creates the update on button
	$ButtonFTP2 = GUICtrlCreateButton("Update Off", 190, 486) ;creates the update off button
...
Func FTP()
	...
	Local $ButtonFolder, $ButtonFile, $InstallFile
	Local $TrimmedFile, $FileCount
	$flag = 0
	...
	If $FTPConnect <> 0 Then
		MsgBox(0, "Alert", "You must specify the path to the specific folder to upload", 20)
		$hChild_2 = GUICreate("Select file/folder", 275, 90, Default, Default, Default, Default, $GUIhandle)
		$ButtonFolder = GUICtrlCreateButton("Folder", 45, 25, 75, 40)
		$ButtonFile = GUICtrlCreateButton("File", 155, 25, 75, 40)
		GUISetState()

		While $msg <> $GUI_EVENT_CLOSE
			$msg = GUIGetMsg()
			If $msg = $ButtonFolder Then
				$InstallFolder = FileSelectFolder("Open", '\C\')
				If @error Then
					$flag = 1
				EndIf
				If $flag = 1 Then
					MsgBox(0, "FTP", "Nothing chosen.. Exiting.", 20)
					_FTP_Close($FTPOpen)
					GUIDelete($hChild_2)
					ExitLoop
					$msg = $GUI_EVENT_CLOSE
				...
				Else
					...
					$msg = $GUI_EVENT_CLOSE
				EndIf
			ElseIf $msg = $ButtonFile Then
				$InstallFile = FileOpenDialog("Open", "\C\", "ALL(*.*)")
				If @error Then
					$flag = 1
				EndIf
				If $flag = 1 Then
					...
					$msg = $GUI_EVENT_CLOSE
				EndIf
				$FileCount = StringSplit($InstallFile, "\")
				$StringCount = $FileCount[0]
				$TrimmedFile = $FileCount[$StringCount]
				$FTPUpload = _FTP_FilePut($FTPConnect, $InstallFile, $TrimmedFile)
				If $FTPUpload <> 0 Then
					MsgBox(0, "FTP", "File(s) successfully uploaded!", 20)
					GUIDelete($hChild_2)
					ExitLoop
